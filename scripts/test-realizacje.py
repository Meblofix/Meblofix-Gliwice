...
def main() -> None:
    ...
    require(len(items) == 9, f"Oczekiwano 9 realizacji, jest {len(items)}")
    require(sum(len(item.get("zdjecia", [])) for item in items) == 56, "Oczekiwano 56 zdjęć logicznych")
    ...
    require(set(card_ids) == set(ids) and len(card_ids) == len(ids), "Homepage nie ma dokładnie dziewięciu kart")
    ...
    print("OK: źródło — 9 realizacji, 56 zdjęć i komplet wariantów")
    print("OK: homepage — 9 lekkich kart bez pełnych opisów, galerii i payloadu zdjęć")
